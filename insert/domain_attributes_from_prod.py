...
class DomainAttributesFromProd:
    ...
    def exists_in_scraping(self, domain: str) -> bool:
        query = """
            SELECT 1
            FROM public.domain_attributes
            WHERE "domain" = %s
    ...
    def insert_into_scraping(self, row: dict):
        ...
        self.logger.debug(
            f"[domain_attributes_from_prod] INSERT en scraping para domain={domain}, domain_id={row['domain_id']}"
        )
        ...
        while True:
            ...
            try:
                for row in rows:
                    ...
                    if self.exists_in_scraping(domain):
                        self.logger.info(
                            f"[domain_attributes_from_prod] Domain ya existe en scraping, no se actualiza: {domain}"
                        )
                    else:
